...
public class RecursiveColumnUpdater {
     ...
     *
     * Order of operations: 1. Recursively move down the tree from each project, updating children
     * with defer/due dates and flags in 'dateDueEffective' and the like. 1.1 Run through each
     * child, and calculate its due soon/overdue columns using the new data. 2. Each parent gets the
     * number of children for each count. 3. Each child determines if it's blocked by checking its
     * parent's 'nextTask' column. 4. Each child updates its parent's and context's 'available'
     * column.
     */
    ...
    /**
     * An 'effective' attribute is where a parent has a certain attribute that should cascade down
     * into all of its children. If a project is due at 9am, then so should all of its children be,
     * and their children, and so on.
     *
     * This function is a flow controller for the recursive function
     * `updateChildEffectiveAttributes()`,
    ...
    public void updateEffectiveAttributes(SQLiteDatabase db) {
        ...
        Cursor projects = mDBHelper.query(db, DatabaseContract.Tasks.TABLE_NAME,
                                          new String[]{DatabaseContract.Tasks.COLUMN_ID.name,
                                                       DatabaseContract.Tasks.COLUMN_DATE_DEFER
                                                               .name,
        ...
        while (projects.moveToNext()) {
            ...
            String deferDate =
                    mDBHelper.getString(projects, DatabaseContract.Tasks.COLUMN_DATE_DEFER.name);
            String dueDate =
                    mDBHelper.getString(projects, DatabaseContract.Tasks.COLUMN_DATE_DUE.name);
            String flagged =
                    mDBHelper.getString(projects, DatabaseContract.Tasks.COLUMN_FLAGGED.name);
    ...
    /**
     * Given a database and some other info, find the task's children (if any), and update them with
     ...
     */
    private void updateChildEffectiveAttributes(SQLiteDatabase db, String id, String dateDefer,
                                                String dateDue, String flagged) {
        ...
        Cursor children = mDBHelper.query(db, DatabaseContract.Tasks.TABLE_NAME,
                                          new String[]{DatabaseContract.Tasks.COLUMN_ID.name,
                                                       ...
                                                       DatabaseContract.Tasks.COLUMN_DATE_DUE.name,
                                                       DatabaseContract.Tasks.COLUMN_FLAGGED.name},
                                          DatabaseContract.Tasks.COLUMN_PARENT_ID.name + "=?",
                                          new String[]{id});

        while (children.moveToNext()) {

            String childID = mDBHelper.getString(children, DatabaseContract.Tasks.COLUMN_ID.name);
            String childDateDefer =
                    mDBHelper.getString(children, DatabaseContract.Tasks.COLUMN_DATE_DEFER.name);
            String childDateDue =
                    mDBHelper.getString(children, DatabaseContract.Tasks.COLUMN_DATE_DUE.name);
            String childFlagged =
                    mDBHelper.getString(children, DatabaseContract.Tasks.COLUMN_FLAGGED.name);
            ...
            if (values.size() > 0) {
                db.update(DatabaseContract.Tasks.TABLE_NAME, values,
                          DatabaseContract.Tasks.COLUMN_ID.name + "=?", new String[]{childID});
            }
        ...
        Cursor dueDates = mDBHelper.query(db, DatabaseContract.Tasks.TABLE_NAME,
                                          new String[]{DatabaseContract.Tasks.COLUMN_DATE_DUE.name,
                                                       DatabaseContract.Tasks
                                                               .COLUMN_DATE_DUE_EFFECTIVE.name},
                                          DatabaseContract.Tasks.COLUMN_ID.name + "=?",
                                          new String[]{id});

        dueDates.moveToNext();

        String normalDate =
                mDBHelper.getString(dueDates, DatabaseContract.Tasks.COLUMN_DATE_DUE.name);
        String effectiveDate = mDBHelper
                .getString(dueDates, DatabaseContract.Tasks.COLUMN_DATE_DUE_EFFECTIVE.name);
    ...
    /**
     * Parents have counts of the numbers of each type of child they have, which are displayed to
     * the user as useful information for organisation.
     */
    public void updateParentCounts(SQLiteDatabase db) {

        Cursor allParents = mDBHelper.query(db, DatabaseContract.Tasks.TABLE_NAME,
                                            new String[]{DatabaseContract.Tasks.COLUMN_ID.name},

                                            // This SQL statement will get all parents.
                                            // How? It finds all entries where their ID is found
                                            // in a child's
        ...
        while (allParents.moveToNext()) {
            ...
            Cursor remaining = db.query(DatabaseContract.Tasks.TABLE_NAME, null,
                                        ...
                                        DatabaseContract.Tasks.COLUMN_RANK.name + " ASC");
            ...
            int countCompleted = mDBHelper.query(db, DatabaseContract.Tasks.TABLE_NAME, null,
                                                 DatabaseContract.Tasks.COLUMN_PARENT_ID.name +
                                                 "=? AND " +
                                                 DatabaseContract.Tasks.COLUMN_DATE_COMPLETED.name +
                                                 " IS NOT NULL", new String[]{id}).getCount();

            int countDueSoon = mDBHelper.query(db, DatabaseContract.Tasks.TABLE_NAME, null,
                                               DatabaseContract.Tasks.COLUMN_PARENT_ID.name +
                                               "=? AND " +
                                               DatabaseContract.Tasks.COLUMN_DUE_SOON.name + "=1",
                                               new String[]{id}).getCount();

            int countOverdue = mDBHelper.query(db, DatabaseContract.Tasks.TABLE_NAME, null,
                                               DatabaseContract.Tasks.COLUMN_PARENT_ID.name +
                                               "=? AND " +
                                               DatabaseContract.Tasks.COLUMN_OVERDUE.name +
                                               "=1 AND " +

                                               // Only get an overdue count for tasks that aren't
                                               // complete

                                               DatabaseContract.Tasks.COLUMN_DATE_COMPLETED.name +
                                               " IS NULL", new String[]{id}).getCount();
            ...
            db.update(DatabaseContract.Tasks.TABLE_NAME, values,
                      DatabaseContract.Tasks.COLUMN_ID + "=?", new String[]{id});
        }
        allParents.close();
