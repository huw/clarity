 ...
 * should be able to only download changed files.
 */
public class GetFilesToDownload extends AsyncTask<Void, Void, List<String>> {

    private static final String TAG = GetFilesToDownload.class.getSimpleName();
     ...
     * referencing your listener interface. Pretty simple.
     */
    ...
    public GetFilesToDownload(TaskListener listener) {
    ...
    private static boolean shouldDownloadFile(MultiStatusResponse response) {
        ...
        // If the content-type header gives us a directory, we're not downloading it
        if (contentType != null) {
            if (contentType.getValue().toString().equals("httpd/unix-directory")) {
        ...
        try {
            ...
            for (MultiStatusResponse response : responses) {
                if (shouldDownloadFile(response)) {
                    ...
                    filesToDownload.add(response.getHref());
